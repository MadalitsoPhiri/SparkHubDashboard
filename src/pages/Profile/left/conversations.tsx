...
const Conversations = () => {
  const initialCon: string[] = [];
  const [visibleCon, setVisibleCon] = useState(initialCon.slice(0, 2));
  const [showMore, setShowMore] = useState(false);

  const toggleVisibility = () => {
    if (showMore) {
      setVisibleCon(initialCon.slice(0, 2));
    } else {
      setVisibleCon(initialCon);
    }
